...
# CSS CUSTOM - APPLE-INSPIRED DESIGN (LIGHT MODE)
st.markdown("""
<style>
    /* Importar fuente Inter (similar a San Francisco de Apple) */
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap');
    
    /* Variables de color - Light Mode */
    :root {
        --primary-bg: #ffffff;
        --secondary-bg: #f5f5f7;
        --card-bg: #ffffff;
...
def create_trend_chart(dates, values, brand_name, color="#FF6B00"):
    """Crea gráfico de tendencias estilo Apple (Light Mode)"""
    fig = go.Figure()
    ...
    fig.add_trace(go.Scatter(
        ...
        fill='tozeroy',
        fillcolor=f'rgba(255, 107, 0, 0.08)',
        hovertemplate='<b>%{y}</b><br>%{x}<extra></extra>'
    ))
    
    # Estilo Apple Light Mode
    fig.update_layout(
...
if search_mode == "🔍 Búsqueda manual":
    ...
    if search_button and search_query:
        ...
        <div class="glass-card">
            <h2 style="margin: 0; font-size: 2rem; color: #1d1d1f;">📊 {search_query}</h2>
            <p style="color: #6e6e73; margin-top: 0.5rem;">Análisis de tendencias multi-país</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    if url_input:
        ...
        if extracted_brand:
            ...
            if st.button(f"🔍 Analizar {extracted_brand}", type="primary"):
                ...
                st.markdown(f"""
                <div class="glass-card">
                    <h2 style="margin: 0; color: #1d1d1f;">📊 {extracted_brand}</h2>
                </div>
                """, unsafe_allow_html=True)
...
# ================================
    ...
    if uploaded_file:
        ...
        if st.button("🔍 Analizar marca", type="primary"):
            ...
            st.markdown(f"""
            <div class="glass-card">
                <h2 style="margin: 0; color: #1d1d1f;">📊 {selected_brand}</h2>
                <p style="color: #6e6e73; margin-top: 0.5rem;">Análisis detallado de tendencias</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        <div class="glass-card" style="text-align: center; padding: 4rem 2rem;">
            <h2 style="color: #6e6e73; font-weight: 400;">📁 Arrastra tu archivo CSV aquí</h2>
            <p style="color: #86868b; margin-top: 1rem;">o usa búsqueda manual para empezar</p>
        </div>
        """, unsafe_allow_html=True)
...
st.markdown("""
<div style="text-align: center; color: #86868b; font-size: 0.9rem; padding: 2rem 0;">
    🔧 Desarrollado para PCComponentes | 🔍 Powered by SerpAPI<br>
    <span style="color: #6e6e73;">Versión 4.0 Premium - Light Mode UI/UX</span>
</div>
""", unsafe_allow_html=True)
